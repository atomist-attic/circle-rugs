import { HandleEvent, EventPlan, LifecycleMessage, DirectedMessage, UserAddress } from '@atomist/rug/operations/Handlers'
import { GraphNode, Match, PathExpression } from '@atomist/rug/tree/PathExpression'
import { EventHandler, Tags } from '@atomist/rug/operations/Decorators'

import { Build } from '@atomist/cortex/Build'

@EventHandler("CircleCIBuilds", "Handle build events from CircleCI",
        ...
        if (build.status == "failed" || build.status == "broken") {
            try {
                if (build.commit.author != null && build.commit.author.person != null) {
                    let body = `Circle build \`#${build.name}\` of \`${owner}/${repo}\` failed after your last commit \`${build.commit.sha}\`: ${build.buildUrl}`
